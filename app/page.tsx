...
import AprenderTesting from "@/components/AprenderTesting";
...
type Fase = "intro" | "aprender" | "practicando" | "evaluando" | "resultado";
...
export default function Home() {
  ...
  async function guardarEstado(id: string, estado: Estado, evidencia?: object) {
    ...
    const { error: upsertError } = await supabase.from("helikon_usuario_progreso").upsert({ usuario_id: usuarioId, nodo_id: id, estado, nivel_dominio: estado === "dominado" ? "confirmado" : "inicial", evidencia: evidencia ? JSON.stringify(evidencia) : null, fecha: new Date().toISOString() }, { onConflict: "usuario_id,nodo_id" });
  ...
  function entrarAlAprendizaje() { setFase("aprender"); }

  async function completarAprendizaje() {
  ...
  async function completarPractica(correctas: number, totalPreguntas: number) {
    ...
    if (correctas !== totalPreguntas) {
      await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", { practicaCompletada: false, ultimoResultado: resultado, refuerzoRequerido: true });
      setFase("resultado");
      return;
    }
    await guardarEstado(nodoSeleccionado.id, "evaluando", { practicaCompletada: true, practicaResultado: 100 });
    setFase("evaluando");
  }
  ...
  async function finalizarEvaluacion(correctas: number, totalPreguntas: number) {
    ...
    const resultado = Math.round((correctas / totalPreguntas) * 100);
    const skills = habilidadesFallidas[nodoSeleccionado.id] ?? [];
    setIntentos((prev) => ({ ...prev, [nodoSeleccionado.id]: numeroIntento }));
    await supabase!.from("helikon_intentos").insert({ usuario_id: usuarioId, nodo_id: nodoSeleccionado.id, tipo: "evaluacion", numero: numeroIntento, resultado, completado: correctas === totalPreguntas, respuestas: { correctas, total: totalPreguntas, tipo: "demostracion", habilidadesFallidas: skills } });
    if (correctas === totalPreguntas) {
    ...
    await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", { intentos: numeroIntento, ultimoResultado: resultado, habilidadesFallidas: skills, refuerzoRequerido: true, oportunidadesRestantes: Math.max(0, MAX_INTENTOS - numeroIntento) });
    setFase("resultado");
  }
  ...
  return <div className="app">
    ...
    <button className="build-link" onClick={() => setCentroAbierto(true)}>⚙ Centro de construcción</button>
    {mision ? <section className="mission"><div className="mission-k">TU SIGUIENTE MISIÓN</div><div className="mission-num">RETO {String(mision.numero).padStart(2, "0")} · {mision.rama}</div><h1>{mision.nombre}</h1><p>{mision.objetivo}</p><div className="mission-meta"><span>📚 aprender</span><span>🛠 practicar</span><span>🧪 evaluar</span></div><button className="btn" onClick={() => abrirNodo(mision.id)}>{estadoDe(mision) === "dominado" ? "Revisar reto" : "Continuar →"}</button></section> : <section className="mission empty"><div className="mission-k">RUTA COMPLETA</div><h1>Has demostrado todas las competencias disponibles.</h1></section>}
    <section className="section-block"><div className="section-head"><div><div className="section-k">MI RUTA</div><div className="section-title">Conocimiento desbloqueado</div></div><button className="map-link" onClick={() => setMapaAbierto(true)}>🗺 Ver mapa completo</button></div><div className="ruta-list">{disponibles.filter((n) => n.id !== mision?.id).map((n) => { const est = estadoDe(n); return <button className={`ruta-item ${est}`} key={n.id} onClick={() => abrirNodo(n.id)}><span className="ruta-num">{String(n.numero).padStart(2, "0")}</span><span className="ruta-name">{n.nombre}</span><span className="ruta-state">{est === "dominado" ? "✓" : est === "disponible" ? "→" : "●"}</span></button>; })}</div>{!disponibles.filter((n) => n.id !== mision?.id).length && <div className="empty-list">Aún no hay más conocimientos desbloqueados.</div>}</section>
    ...
    {centroAbierto && <CentroConstruccion onClose={() => setCentroAbierto(false)} />}
    <div className={`overlay ${nodoSeleccionado ? "open" : ""}`}>{nodoSeleccionado && <div className="sheet"><button className="close" onClick={() => setSeleccionado(null)}>✕</button><Reto nodo={nodoSeleccionado} nodos={nodos} fase={fase} evaluacion={evaluaciones[nodoSeleccionado.id] ?? crearEvaluacion(nodoSeleccionado, nodos)} intento={intentos[nodoSeleccionado.id] ?? 0} onIntro={entrarAlAprendizaje} onComenzar={completarAprendizaje} onPractica={completarPractica} onEvaluar={finalizarEvaluacion} onReforzar={() => setFase("aprender")} onClose={() => setSeleccionado(null)} dominado={dominados.has(nodoSeleccionado.id)} habilidadesFallidas={habilidadesFallidas[nodoSeleccionado.id] ?? []} /></div>}</div>
  </div>;
}

function crearEvaluacion(nodo: Nodo, nodos: Nodo[]): EvaluacionNodo {
  const otros = nodos.filter((n) => n.id !== nodo.id);
  const distractores = otros.slice(0, 2);
  ...
  return { nodo_id: nodo.id, practica: [make("¿Cuál es la capacidad que este reto busca desarrollar?", nodo.objetivo, opciones(nodo.objetivo, "objetivo")), make("¿Qué vas a construir o practicar primero?", nodo.construir, opciones(nodo.construir, "construir"))], preguntas: [make("¿Qué objetivo debes poder demostrar al terminar?", nodo.objetivo, opciones(nodo.objetivo, "objetivo")), make("¿Qué situación representa el fallo que debes saber detectar?", nodo.romper, opciones(nodo.romper, "romper")), make("¿Qué idea explica la resolución del reto?", nodo.resolver, opciones(nodo.resolver, "resolver"))] };
}

function Reto({ nodo, nodos, fase, evaluacion, intento, onIntro, onComenzar, onPractica, onEvaluar, onReforzar, onClose, dominado, habilidadesFallidas }: { nodo: Nodo; nodos: Nodo[]; fase: Fase; evaluacion: EvaluacionNodo; intento: number; onIntro: () => void; onComenzar: () => void; onPractica: (correctas: number, total: number) => void; onEvaluar: (correctas: number, total: number) => void; onReforzar: () => void; onClose: () => void; dominado: boolean; habilidadesFallidas: string[] }) {
  const prereqNames = nodo.prerequisitos.map((p) => nodos.find((x) => x.id === p)?.nombre).filter(Boolean).join(", ") || "ninguno";
  if (fase === "intro") return <IntroduccionReto numero={nodo.numero} rama={nodo.rama} nombre={nodo.nombre} objetivo={nodo.objetivo} onStart={onIntro} />;
  ...
  if (fase === "aprender" && nodo.id === "TESTB") return <AprenderTesting onComplete={onComenzar} />;
  ...
  if (fase === "resultado") {
    const exito = dominado;
    const agotado = !exito && intento >= MAX_INTENTOS;
    return <div><div className={`sheet-eyebrow ${exito ? "ok" : "danger"}`}>{exito ? "COMPETENCIA DEMOSTRADA" : agotado ? "PAUSA DE DOMINIO" : "REFUERZO NECESARIO"}</div><div className={exito ? "success-mark" : "failure-mark"}>{exito ? "✓" : "!"}</div><div className="sheet-title">{exito ? "Ya puedes usar este conocimiento." : agotado ? "Necesitamos consolidar la base antes de volver a evaluar." : "Todavía hay una parte que debemos reforzar."}</div>{!exito && habilidadesFallidas.length > 0 && <div className="learn-box"><div className="k">HABILIDADES A REFORZAR</div><div className="v">{habilidadesFallidas.join(" · ")}</div></div>}<p className="result-text">{exito ? "La competencia queda registrada y el siguiente conocimiento se desbloquea automáticamente." : agotado ? "Has usado las oportunidades disponibles. Vuelve a recorrer el aprendizaje y practica de nuevo antes de intentar demostrarla." : `Intento ${intento} de ${MAX_INTENTOS}. Volveremos al aprendizaje para trabajar la debilidad detectada.`}</p><button className="btn" onClick={exito ? onClose : onReforzar}>{exito ? "Continuar →" : "Volver a aprender →"}</button></div>;
  }
...
function Cuestionario({ titulo, preguntas, boton, onFinish }: { titulo: string; preguntas: Pregunta[]; boton: string; onFinish: (correctas: number, total: number) => void }) {
  ...
  const complete = preguntas.length > 0 && preguntas.every((_, i) => seleccion[i] !== undefined);
  const enviar = () => onFinish(preguntas.reduce((a, p, i) => a + (seleccion[i] === p.correcta ? 1 : 0), 0), preguntas.length);
  return <div><div className="sheet-eyebrow">{titulo}</div><div className="sheet-title">Comprueba tu comprensión</div><div className="question-list">{preguntas.map((p, i) => <div className="question" key={i}><div className="question-n">{String(i + 1).padStart(2, "0")}</div><div className="question-text">{p.pregunta}</div><div className="options">{p.opciones.map((o, j) => <button className={`option ${seleccion[i] === j ? "selected" : ""}`} key={j} onClick={() => setSeleccion((s) => ({ ...s, [i]: j }))}>{o}</button>)}</div></div>)}</div><button className="btn" disabled={!complete} onClick={enviar}>{boton}</button></div>;
}

function MapaCompleto({ nodos, estadoDe, onClose, onOpen }: { nodos: Nodo[]; estadoDe: (n: Nodo) => Estado; onClose: () => void; onOpen: (id: string) => void }) {
  const ramas = [...new Set(nodos.map((n) => n.rama))];
  return <div className="map-overlay"><div className="map-sheet"><button className="close" onClick={onClose}>✕</button><div className="sheet-eyebrow">MAPA COMPLETO</div><div className="sheet-title">Toda la ruta</div><p className="map-help">Aquí puedes explorar conocimientos bloqueados. La ruta normal solo muestra lo que puedes trabajar ahora.</p>{ramas.map((rama) => <div className="map-rama" key={rama}><div className="rama-title">{rama}</div><div className="map-grid">{nodos.filter((n) => n.rama === rama).map((n) => { const est = estadoDe(n); return <button className={`map-node ${est}`} key={n.id} disabled={est === "bloqueado"} onClick={() => onOpen(n.id)}><span>RETO {String(n.numero).padStart(2, "0")}</span><b>{n.nombre}</b><small>{est === "dominado" ? "✓ dominado" : est === "disponible" ? "disponible" : "🔒 bloqueado"}</small></button>; })}</div></div>)}</div></div>;
}
