...
export default function Home() {
    ...
    async function cargarProgreso() {
      ...
      (data ?? []).forEach((r) => {
        ...
        try {
          ...
          if (Array.isArray(evidence?.habilidadesFallidas)) nextSkills[r.nodo_id] = evidence.habilidadesFallidas;
        } catch {}
      });
  ...
  async function completarPractica(correctas: number, totalPreguntas: number) {
    ...
    if (correctas !== totalPreguntas) { await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", { practicaCompletada: false, ultimoResultado: Math.round((correctas / totalPreguntas) * 100) }); setFase("resultado"); return; }
    await guardarEstado(nodoSeleccionado.id, "evaluando", { practicaCompletada: true, practicaResultado: 100 }); setFase("evaluando");
  }
