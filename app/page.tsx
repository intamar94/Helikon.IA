...
export default function Home() {
  ...
  async function finalizarEvaluacion(correctas: number, totalPreguntas: number) { if (!nodoSeleccionado || !usuarioId || (intentos[nodoSeleccionado.id] ?? 0) >= MAX_INTENTOS) return; const numeroIntento = (intentos[nodoSeleccionado.id] ?? 0) + 1; const resultado = Math.round((correctas / totalPreguntas) * 100); let skills: string[] = habilidadesFallidas[nodoSeleccionado.id] ?? []; try { const raw = localStorage.getItem("helikon:last-failed-skills"); if (["L","V","F"].includes(nodoSeleccionado.id)) skills = raw ? JSON.parse(raw) : skills; } catch {} setIntentos((prev) => ({ ...prev, [nodoSeleccionado.id]: numeroIntento })); setHabilidadesFallidas((prev) => ({ ...prev, [nodoSeleccionado.id]: skills })); await supabase!.from("helikon_intentos").insert({ usuario_id: usuarioId, nodo_id: nodoSeleccionado.id, tipo: "evaluacion", numero: numeroIntento, resultado, completado: correctas === totalPreguntas, respuestas: { correctas, total: totalPreguntas, tipo: ["L","V","F"].includes(nodoSeleccionado.id) ? "demostracion" : "cuestionario", habilidadesFallidas: skills } }); if (correctas === totalPreguntas) { await guardarEstado(nodoSeleccionado.id, "dominado", { intentos: numeroIntento, resultado: 100, demostrado: ["aprendizaje", "practica", "evaluacion", "transferencia"], evidencia: "demostracion", habilidadesFallidas: [] }); setFase("resultado"); return; } await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", { intentos: numeroIntento, ultimoResultado: resultado, habilidadesFallidas: skills, refuerzoRequerido: true, oportunidadesRestantes: Math.max(0, MAX_INTENTOS - numeroIntento) }); setFase("resultado"); }
...
function Reto({ nodo, nodos, fase, evaluacion, intento, onComenzar, onPractica, onEvaluar, onReforzar, dominado, habilidadesFallidas }: { nodo: Nodo; nodos: Nodo[]; fase: Fase; evaluacion: EvaluacionNodo; intento: number; onComenzar: () => void; onPractica: (correctas: number, total: number) => void; onEvaluar: (correctas: number, total: number) => void; onReforzar: () => void; dominado: boolean; habilidadesFallidas: string[] }) {
  ...
  if (fase === "evaluando" && nodo.id === "F") return <EvaluacionFunciones onFinish={onEvaluar} />;
  ...
  if (fase === "resultado") { if (dominado) return <div><div className="success-mark">✓</div><div className="sheet-eyebrow ok">COMPETENCIA CONFIRMADA</div><div className="sheet-title">{nodo.nombre}</div><p className="result-text">Has demostrado la competencia mediante aprendizaje, práctica, evaluación y transferencia. El siguiente nodo ya puede desbloquearse.</p><button className="btn" onClick={onReforzar}>Revisar lo aprendido</button></div>; const skillsText = habilidadesFallidas.length ? `La evaluación detectó que debes reforzar: ${habilidadesFallidas.join(", ")}.` : "Helikon detectó que todavía falta evidencia suficiente de dominio."; const agotado = intento >= MAX_INTENTOS; return <div><div className="failure-mark">!</div><div className="sheet-eyebrow danger">{agotado ? "LÍMITE DE INTENTOS" : "NECESITA REFUERZO"}</div><div className="sheet-title">{agotado ? "La competencia sigue sin estar demostrada" : "Todavía no hay dominio suficiente"}</div><p className="result-text">{agotado ? `Has utilizado los ${MAX_INTENTOS} intentos disponibles. La ruta no se desbloquea hasta que exista evidencia suficiente de dominio.` : `${skillsText} Helikon te devuelve al contenido para trabajar esa habilidad con una situación diferente antes de permitir otro intento.`}</p><div className="evidence-note">Intento {Math.min(intento, MAX_INTENTOS)}/{MAX_INTENTOS} · el siguiente avance depende de demostrar la competencia.</div>{!agotado && <button className="btn" onClick={onReforzar}>Volver a aprender</button>}</div>; }
...
function Cuestionario({ titulo, preguntas, boton, onFinish }: { titulo: string; preguntas: Pregunta[]; boton: string; onFinish: (correctas: number, total: number) => void }) { const [respuestas, setRespuestas] = useState<Record<number, number>>({}); const completo = preguntas.length > 0 && preguntas.every((_, i) => respuestas[i] !== undefined); function finalizar() { const correctas = preguntas.reduce((sum, p, i) => sum + (respuestas[i] === p.correcta ? 1 : 0), 0); onFinish(correctas, preguntas.length); } return <><div className="sheet-eyebrow">{titulo}</div><div className="sheet-title">Demuestra lo que entiendes</div><div className="question-list">{preguntas.map((p, i) => <div className="question" key={i}><div className="question-n">0{i + 1}</div><div className="question-text">{p.pregunta}</div><div className="options">{p.opciones.map((op, j) => <button key={j} className={`option ${respuestas[i] === j ? "selected" : ""}`} onClick={() => setRespuestas((r) => ({ ...r, [i]: j }))}>{op}</button>)}</div></div>)}</div><button className="btn" disabled={!completo} onClick={finalizar}>{boton}</button></>; }

function MapaCompleto({ nodos, estadoDe, onClose, onOpen }: { nodos: Nodo[]; estadoDe: (n: Nodo) => Estado; onClose: () => void; onOpen: (id: string) => void }) { const ramas = useMemo(() => { const grupos: Record<string, Nodo[]> = {}; nodos.forEach((n) => (grupos[n.rama] ??= []).push(n)); return grupos; }, [nodos]); return <div className="map-overlay"><div className="map-sheet"><div className="map-head"><div><div className="section-k">EXPLORACIÓN</div><div className="sheet-title">Mapa completo</div></div><button className="close" onClick={onClose}>✕</button></div><p className="map-help">Aquí puedes explorar lo bloqueado. El mapa no permite saltar la ruta: solo muestra cómo se conectan las competencias.</p>{Object.entries(ramas).map(([rama, items]) => <div className="map-rama" key={rama}><div className="rama-title">{rama}</div><div className="map-grid">{items.map((n) => { const e = estadoDe(n); return <button key={n.id} className={`map-node ${e}`} onClick={() => e !== "bloqueado" && onOpen(n.id)}><span>{String(n.numero).padStart(2, "0")}</span><b>{n.nombre}</b><small>{e === "dominado" ? "✓ dominado" : e === "disponible" ? "desbloqueado" : "bloqueado"}</small></button>; })}</div></div>)}</div></div>; }
