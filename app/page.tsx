...
export default function Home() {
  ...
  function abrirNodo(id: string) { const nodo = nodos?.find((n) => n.id === id); if (!nodo || estadoDe(nodo) === "bloqueado") return; setSeleccionado(id); setFase("intro"); }
  ...
  async function completarAprendizaje() { if (!nodoSeleccionado) return; if (!dominados.has(nodoSeleccionado.id) && (intentos[nodoSeleccionado.id] ?? 0) >= MAX_INTENTOS) { setFase("resultado"); return; } await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", { aprendizajeCompletado: true, fecha: new Date().toISOString() }); setFase("practicando"); }
  async function completarPractica(correctas: number, totalPreguntas: number) { if (!nodoSeleccionado || !usuarioId) return; const resultado = Math.round((correctas / totalPreguntas) * 100); if (correctas !== totalPreguntas) { await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", { practicaCompletada: false, ultimoResultado: resultado, refuerzoRequerido: true }); setFase("resultado"); return; } await guardarEstado(nodoSeleccionado.id, "evaluando", { practicaCompletada: true, practicaResultado: 100 }); setFase("evaluando"); }
  async function finalizarEvaluacion(correctas: number, totalPreguntas: number) { if (!nodoSeleccionado || !usuarioId || (intentos[nodoSeleccionado.id] ?? 0) >= MAX_INTENTOS) return; const numeroIntento = (intentos[nodoSeleccionado.id] ?? 0) + 1; const resultado = Math.round((correctas / totalPreguntas) * 100); const skills = habilidadesFallidas[nodoSeleccionado.id] ?? []; setIntentos((prev) => ({ ...prev, [nodoSeleccionado.id]: numeroIntento })); await supabase!.from("helikon_intentos").insert({ usuario_id: usuarioId, nodo_id: nodoSeleccionado.id, tipo: "evaluacion", numero: numeroIntento, resultado, completado: correctas === totalPreguntas, respuestas: { correctas, total: totalPreguntas, tipo: "demostracion", habilidadesFallidas: skills } }); if (correctas === totalPreguntas) { await guardarEstado(nodoSeleccionado.id, "dominado", { intentos: numeroIntento, resultado: 100, demostrado: ["aprendizaje", "practica", "evaluacion", "transferencia"], evidencia: "demostracion", habilidadesFallidas: [] }); setFase("resultado"); return; } await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", { intentos: numeroIntento, ultimoResultado: resultado, habilidadesFallidas: skills, refuerzoRequerido: true, oportunidadesRestantes: Math.max(0, MAX_INTENTOS - numeroIntento) }); setFase("resultado"); }
