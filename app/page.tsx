...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      if (nodosResult.error || aristasResult.error) {
        setError((nodosResult.error || aristasResult.error)?.message ?? "Error cargando datos");
        return;
      }
      ...
      const completos: Nodo[] = (nodosResult.data ?? []).map((n) => ({
        ...n,
      ...
      setNodos(completos);
      setEvaluaciones(evaluacionesDB);
    }
    ...
    async function cargarProgreso() {
      ...
      (data ?? []).forEach((r) => {
        ...
        try {
          ...
          // Evidencia antigua no JSON: se conserva, sin romper la carga.
        }
      });

      setEstados(nextStates);
      setDominados(nextDominados);
      setIntentos(nextAttempts);
    }
  ...
  const estadoDe = (nodo: Nodo): Estado => {
    ...
    if (estados[nodo.id] === "en_aprendizaje" || estados[nodo.id] === "practicando" || estados[nodo.id] === "evaluando") {
      return estados[nodo.id];
    }
    return nodo.prerequisitos.every((p) => dominados.has(p)) ? "disponible" : "bloqueado";
  };

  const disponibles = useMemo(
    () => (nodos ?? []).filter((n) => estadoDe(n) !== "bloqueado"),
    [nodos, dominados, estados]
  );
  ...
  async function comenzar() {
    ...
    await guardarEstado(nodoSeleccionado.id, "en_aprendizaje");
    setFase("practicando");
  }
  ...
  async function completarPractica(correctas: number, totalPreguntas: number) {
    ...
    await guardarEstado(nodoSeleccionado.id, "evaluando");
    setFase("evaluando");
  }
  ...
  async function finalizarEvaluacion(correctas: number, totalPreguntas: number) {
    ...
    await supabase!.from("helikon_intentos").insert({
      usuario_id: usuarioId,
      nodo_id: nodoSeleccionado.id,
      tipo: "evaluacion",
      numero: numeroIntento,
      resultado: Math.round((correctas / totalPreguntas) * 100),
      completado: correctas === totalPreguntas,
      respuestas: { correctas, total: totalPreguntas },
    });

    if (correctas === totalPreguntas) {
      await guardarEstado(nodoSeleccionado.id, "dominado", {
        intentos: numeroIntento,
        resultado: 100,
        demostrado: ["practica", "evaluacion"],
      });
      setFase("resultado");
      return;
    }
    ...
    await guardarEstado(nodoSeleccionado.id, "en_aprendizaje", {
      intentos: numeroIntento,
      ultimoResultado: Math.round((correctas / totalPreguntas) * 100),
    });
    setFase("resultado");
  }
  ...
  return (
    <div className="app">
      <header className="topbar">
        <div className="brand"><b>Helikon.IA</b><span> · Ingeniería de Software</span></div>
        <div className="stat">{done}/{total}</div>
      </header>

      <div className="progressbar" aria-label="progreso global">
        <div className="progressfill" style={{ width: `${total ? (done / total) * 100 : 0}%` }} />
      </div>
      ...
      {mision ? (
        <section className="mission">
          ...
          <div className="mission-meta">
            <span>📚 aprender</span><span>🛠 practicar</span><span>🧪 evaluar</span>
          </div>
          <button className="btn" onClick={() => abrirNodo(mision.id)}>
            {estadoDe(mision) === "dominado" ? "Revisar reto" : "Continuar →"}
          </button>
        </section>
      ) : (
        <section className="mission empty"><div className="mission-k">RUTA COMPLETA</div><h1>Has demostrado todas las competencias disponibles.</h1></section>
      )}
      ...
      <section className="section-block">
        ...
        <div className="ruta-list">
          {disponibles.filter((n) => n.id !== mision?.id).map((n) => {
            const est = estadoDe(n);
            return <button className={`ruta-item ${est}`} key={n.id} onClick={() => abrirNodo(n.id)}>
              <span className="ruta-num">{String(n.numero).padStart(2, "0")}</span>
              <span className="ruta-name">{n.nombre}</span>
              <span className="ruta-state">{est === "dominado" ? "✓" : est === "disponible" ? "→" : "●"}</span>
            </button>;
          })}
      ...
      <div className={`overlay ${nodoSeleccionado ? "open" : ""}`}>
        {nodoSeleccionado && (
          <div className="sheet">
            <button className="close" onClick={() => setSeleccionado(null)}>✕</button>
            <Reto
              nodo={nodoSeleccionado}
              nodos={nodos}
              fase={fase}
              evaluacion={evaluaciones[nodoSeleccionado.id] ?? crearEvaluacion(nodoSeleccionado, nodos)}
              intento={intentos[nodoSeleccionado.id] ?? 0}
              onComenzar={comenzar}
              onPractica={completarPractica}
              onEvaluar={finalizarEvaluacion}
              onReforzar={() => setFase("aprender")}
              dominado={dominados.has(nodoSeleccionado.id)}
            />
          </div>
        )}
      </div>
...
function crearEvaluacion(nodo: Nodo, nodos: Nodo[]): EvaluacionNodo {
  ...
  const opciones = (correcta: string | null, campo: keyof Nodo) => {
    const valores = [correcta, ...distractores.map((n) => n[campo] as string | null)].filter(Boolean) as string[];
    return [...new Set(valores)].slice(0, 3);
  };

  const make = (pregunta: string, correcta: string | null, opts: string[]): Pregunta => ({
    pregunta,
    opciones: opts,
    correcta: Math.max(0, opts.indexOf(correcta ?? "")),
  });

  return {
    nodo_id: nodo.id,
    practica: [
      make("¿Cuál es la capacidad que este reto busca desarrollar?", nodo.objetivo, opciones(nodo.objetivo, "objetivo")),
      make("¿Qué vas a construir o practicar primero?", nodo.construir, opciones(nodo.construir, "construir")),
    ],
    preguntas: [
      make("¿Qué objetivo debes poder demostrar al terminar?", nodo.objetivo, opciones(nodo.objetivo, "objetivo")),
      make("¿Qué situación representa el fallo que debes saber detectar?", nodo.romper, opciones(nodo.romper, "romper")),
      make("¿Qué idea explica la resolución del reto?", nodo.resolver, opciones(nodo.resolver, "resolver")),
    ],
  };
}
...
function Reto({
  ...
  intento: number;
  onComenzar: () => void;
  onPractica: (correctas: number, total: number) => void;
  onEvaluar: (correctas: number, total: number) => void;
  onReforzar: () => void;
  dominado: boolean;
}) {
  ...
  if (fase === "practicando") {
    return <Cuestionario titulo="PRÁCTICA GUIADA" preguntas={evaluacion.practica} boton="Pasar a evaluación →" onFinish={onPractica} />;
  }

  if (fase === "evaluando") {
    return <Cuestionario titulo={`EVALUACIÓN · INTENTO ${Math.min(intento + 1, MAX_INTENTOS)}/${MAX_INTENTOS}`} preguntas={evaluacion.preguntas} boton="Evaluar resultado" onFinish={onEvaluar} />;
  }

  if (fase === "resultado") {
    if (dominado) {
      return <div><div className="success-mark">✓</div><div className="sheet-eyebrow ok">COMPETENCIA CONFIRMADA</div><div className="sheet-title">{nodo.nombre}</div><p className="result-text">Has demostrado la competencia mediante práctica y evaluación. El siguiente nodo ya puede desbloquearse.</p><button className="btn" onClick={onReforzar}>Revisar lo aprendido</button></div>;
    }
...
function Cuestionario({ titulo, preguntas, boton, onFinish }: { titulo: string; preguntas: Pregunta[]; boton: string; onFinish: (correctas: number, total: number) => void }) {
  ...
  function finalizar() {
    const correctas = preguntas.reduce((sum, p, i) => sum + (respuestas[i] === p.correcta ? 1 : 0), 0);
    onFinish(correctas, preguntas.length);
  }

  return <>
    <div className="sheet-eyebrow">{titulo}</div>
    <div className="sheet-title">Demuestra lo que entiendes</div>
    <div className="question-list">
      {preguntas.map((p, i) => <div className="question" key={i}>
        <div className="question-n">0{i + 1}</div>
        <div className="question-text">{p.pregunta}</div>
        <div className="options">{p.opciones.map((op, j) => <button key={j} className={`option ${respuestas[i] === j ? "selected" : ""}`} onClick={() => setRespuestas((r) => ({ ...r, [i]: j }))}>{op}</button>)}</div>
      </div>)}
    </div>
    <button className="btn" disabled={!completo} onClick={finalizar}>{boton}</button>
  </>;
}
...
function MapaCompleto({ nodos, estadoDe, onClose, onOpen }: { nodos: Nodo[]; estadoDe: (n: Nodo) => Estado; onClose: () => void; onOpen: (id: string) => void }) {
  const ramas = useMemo(() => {
    ...
    nodos.forEach((n) => (grupos[n.rama] ??= []).push(n));
    return grupos;
  }, [nodos]);

  return <div className="map-overlay">
    <div className="map-sheet">
      <div className="map-head"><div><div className="section-k">EXPLORACIÓN</div><div className="sheet-title">Mapa completo</div></div><button className="close" onClick={onClose}>✕</button></div>
      <p className="map-help">Aquí puedes explorar lo bloqueado. El mapa no permite saltar la ruta: solo muestra cómo se conectan las competencias.</p>
      {Object.entries(ramas).map(([rama, items]) => <div className="map-rama" key={rama}><div className="rama-title">{rama}</div><div className="map-grid">{items.map((n) => { const e = estadoDe(n); return <button key={n.id} className={`map-node ${e}`} onClick={() => e !== "bloqueado" && onOpen(n.id)}><span>{String(n.numero).padStart(2, "0")}</span><b>{n.nombre}</b><small>{e === "dominado" ? "✓ dominado" : e === "disponible" ? "desbloqueado" : "bloqueado"}</small></button>; })}</div></div>)}
    </div>
  </div>;
}
