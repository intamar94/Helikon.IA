...
import ReforzarVariables from "@/components/ReforzarVariables";
...
export default function AprenderVariables({ onComplete }: Props) {
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(parsed) && parsed.length) setRefuerzo(parsed);
    } catch {}
  }, []);
  ...
  if (refuerzo) return <ReforzarVariables skills={refuerzo} onComplete={onComplete} />;
