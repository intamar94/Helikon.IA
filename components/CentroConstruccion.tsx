...
function etapas(estado: Estado, hasta = estado === "validado" ? 9 : 0): Etapa[] {
  return NOMBRES.map((nombre, i) => ({
    nombre,
    estado: estado === "validado" ? "validado" : i < hasta ? "validado" : i === hasta ? "en_construccion" : "pendiente",
  }));
}
...
function pct(reto: Reto) {
  return Math.round((reto.etapas.filter((e) => e.estado === "validado").length / reto.etapas.length) * 100);
}

function icon(estado: Estado) {
  return estado === "validado" ? "✓" : estado === "en_construccion" ? "◐" : estado === "requiere_correccion" ? "⚠" : "○";
}
    ...
    <div className="build-overlay">
      <div className="build-sheet">
        ...
        <div className="build-list">
          {RETOS.map((reto) => (
            <details key={reto.numero} open={reto.numero === 5} className="build-reto">
              <summary><span className="build-reto-id">{String(reto.numero).padStart(2, "0")}</span><b>{reto.nombre}</b><span className="build-pct">{pct(reto)}%</span></summary>
              <div className="build-stages">
                {reto.etapas.map((etapa) => <div className="build-stage" key={etapa.nombre}><span className={`build-state ${etapa.estado}`}>{icon(etapa.estado)}</span><span>{etapa.nombre}</span><span className="build-stage-state">{etapa.estado.replace("_", " ")}</span></div>)}
              </div>
            </details>
          ))}
        </div>
