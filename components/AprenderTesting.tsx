...
const tarjetas = [
  ...
  { titulo: "2 · El caso normal no basta", texto: "Un buen test también busca límites y fallos: cero, negativos, valores vacíos, tipos inesperados y condiciones extremas.", ejemplo: "entrada = 0 → ¿qué debería ocurrir?" },
  { titulo: "3 · Un test debe aislar el problema", texto: "Cuando falla, queremos saber qué comportamiento se rompió. Por eso conviene probar una idea pequeña y observable.", ejemplo: "1 expectativa clara → 1 fallo fácil de localizar" },
];
...
export default function AprenderTesting({ onComplete }: Props) {
  ...
  const pregunta = actual === 0 ? "¿Qué hace que una prueba sea útil?" : actual === 1 ? "¿Qué tipo de caso puede revelar un fallo que el caso normal no encuentra?" : "¿Por qué conviene que un test falle de forma fácil de localizar?";
  const opciones = actual === 0 ? ["Una expectativa concreta", "Probar muchas cosas a la vez"] : actual === 1 ? ["Un caso límite", "Solo el valor habitual"] : ["Porque aísla el comportamiento roto", "Porque evita escribir código"];
  ...
  return <section style={{ color: "var(--text)" }}>
    ...
    <div style={{ marginTop: 14, border: "1px solid var(--line)", borderRadius: 12, padding: 14 }}>
      ...
      <p style={{ margin: "0 0 10px", fontSize: 13, lineHeight: 1.5 }}>{pregunta}</p>
      {opciones.map((op) => <button key={op} onClick={() => setRespuesta(op)} style={{ display: "block", width: "100%", textAlign: "left", padding: "10px 11px", marginTop: 7, borderRadius: 8, border: `1px solid ${respuesta === op ? "var(--accent)" : "var(--line)"}`, background: respuesta === op ? "rgba(110,140,255,.1)" : "transparent", color: "var(--text)" }}>{op}</button>)}
